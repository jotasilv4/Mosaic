...
class EmojiPicker(Box):
        ...
        self.header_box = Box(
            ...
            children=[
                ...
                Button(
                    name="close-button",
                    can_focus=False,
                    child=icons.image("cancel", widget_name="close-label", size=20),
                    tooltip_text="Exit",
    ...
    def _load_emoji_data(self):
        emoji_data = {}
        emoji_file_path = os.path.expanduser(f"~/.config/{data.APP_NAME_CAP}/assets/emoji.json")
    ...
    def copy_emoji_to_clipboard(self, emoji_char: str):
        try:
            subprocess.run(["xclip", "-selection", "clipboard"], input=emoji_char.encode('utf-8'), check=True)
